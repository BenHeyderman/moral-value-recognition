...
class BespokeFeatures:
    class Melody:
        ...
        def get_note_diffs(midi_notes):
            """
            Computes differences between consecutive notes.
        ...
        def melody_stats(note_diffs):
            """
            Computes statistical features of the melody.
        ...
        @staticmethod
        def get_melody_features(filename, smooth=0.25, minduration=0.1, stop_sec=None, fs = 44100, hop = 128):
            """
            Extracts melody features from an audio file.
...
class DatasetConstructor:
    ...
    @staticmethod
    def extract_from_folder(audio_folder, file_extension="mp3", save_dataset=True, dataset_filepath="dataset.pkl", save_checkpoint=True, checkpoint_interval=100):
        """
        ...
        for audio_filepath in audio_filepaths:
            ...
            # save checkpoint
            if save_checkpoint and (count % checkpoint_interval == 0):
                print("Save checkpoint reached.")
                DatasetConstructor.save_dataset(dataset_filepath, master_features)
    ...
    def dict2df(dataset_dict, included_categories=[], save_dataset=True, dataset_filename="dataset.csv"):
        ...
        # Make all columsn individual values
